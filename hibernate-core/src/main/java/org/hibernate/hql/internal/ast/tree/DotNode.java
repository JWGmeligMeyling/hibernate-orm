...
///////////////////////////////////////////////////////////////////////////////
	 ...
	 * <p/>
	 *
	 * This method resolves the {@code propertyName} as an property of the entity type at the
	 * {@link #propertyPath} relative to the {@link #getFromElement() FromElement}.
	 * The implementation does so by invoking {@link EntityPersister#getPropertyType(String)},
	 * which will resolve the property path against the entitiy's {@link org.hibernate.persister.entity.PropertyMapping}.
	 * On initialisation of the {@code EntityPersister}, this {@code PropertyMapping} is filled with
	 ...
	 * {@code FromElement}, and as such the entity property can be dereferenced (optimised) in the final query.
	 *
	 * <p/>
	...
	private boolean isPropertyEmbeddedInJoinProperties(String propertyName) {
		EntityPersister entityPersister = this.getFromElement().getEntityPersister();
		String propertyPath = String.join( ".", this.propertyPath, propertyName );
		try {
			Type propertyType = entityPersister.getPropertyType( propertyPath );
